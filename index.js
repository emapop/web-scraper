...
const urlSearchOlx = `https://www.olx.ro/electronice-si-electrocasnice/telefoane-mobile/cluj-napoca/q-${phoneQueriedOlx}/?currency=RON&search%5Border%5D=filter_float_price:asc`;
...
axios(urlSearchOlx)
    .then(response => {
        ...
        $('.css-1sw7q4x', html).each(function(){
           ...
           let price = $(this).find('.css-10b0gli').children().remove().end().text();
           ...
           if (price || date) {
            articles.push({title, price, location, date, url})
           }
